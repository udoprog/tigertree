...
#define t1 (tigerhash_table)
#define t2 (tigerhash_table+256)
#define t3 (tigerhash_table+256*2)
#define t4 (tigerhash_table+256*3)
...
tigerhash_new()
{
  tigerhash *new = malloc(sizeof(tigerhash));
  new->pos = 0;
  new->res[0] = _ULL(0x0123456789ABCDEF);
  new->res[1] = _ULL(0xFEDCBA9876543210);
  new->res[2] = _ULL(0xF096A5B4C3B2E187);
  return new;
}
...
tigerhash_result(th)
  tigerhash *th;
{
  return (uint8_t*) th->res;
}

/* The compress function is a function. Requires smaller cache? */
void
tigerhash_compress(th, data, state)
  tigerhash *th;
  const uint64_t* data;
  uint64_t state[3];
{
	tiger_compress_macro(((const uint64_t*)data), ((uint64_t*)state));
}

void
tigerhash_update(th, data, length)
  tigerhash *th;
  const void* data;
  size_t length;
{
	size_t tmppos = (uint32_t)((th->pos) & (TIGER_BLOCK_SIZE - 1));

#ifdef TIGER_BIG_ENDIAN
	uint8_t buf[TIGER_BLOCK_SIZE];
	int j;
#endif
	const uint8_t* str = (const uint8_t*)data;
	// First empty tmp buffer if possible
	if(tmppos > 0) {
		size_t n = min(length, TIGER_BLOCK_SIZE-tmppos);
		memcpy(th->tmp + tmppos, str, n);
		str += n;
		th->pos += n;
		length -= n;

		if((tmppos + n) == TIGER_BLOCK_SIZE) {
#ifdef TIGER_BIG_ENDIAN
			for(j=0; j<TIGER_BLOCK_SIZE; j++)
				buf[j^7]=((uint8_t*)th->tmp)[j];
			tiger_compress_macro(((uint64_t*)buf), th->res);
#else
			tiger_compress_macro(((uint64_t*)th->tmp), th->res);
#endif
			tmppos = 0;
		}
	}

	// So, now either th->tmp is empty or all data has been consumed...
	assert(length == 0 || tmppos == 0);

	// Process the bulk of data
	while(length>=TIGER_BLOCK_SIZE) {
#ifdef TIGER_BIG_ENDIAN
		for(j=0; j<TIGER_BLOCK_SIZE; j++)
			buf[j^7]=((uint8_t*)str)[j];
		tiger_compress_macro(((uint64_t*)buf), th->res);
#else
		tiger_compress_macro(((uint64_t*)str), th->res);
#endif
		str += TIGER_BLOCK_SIZE;
		th->pos += TIGER_BLOCK_SIZE;
		length -= TIGER_BLOCK_SIZE;
	}

	// Copy the rest to the th->tmp buffer
	memcpy(th->tmp, str, length);
	th->pos += length;
}

uint8_t*
tigerhash_finalize(th)
  tigerhash *th;
{
	size_t tmppos = (size_t)(th->pos & (TIGER_BLOCK_SIZE - 1));
#ifdef TIGER_BIG_ENDIAN
	uint8_t buf[TIGER_BLOCK_SIZE];
	int j;
#endif
	// Tmp buffer always has at least one th->pos, otherwise it would have
	// been processed in update()

	th->tmp[tmppos++] = 0x01;

	if(tmppos > (TIGER_BLOCK_SIZE - sizeof(uint64_t))) {
		memset(th->tmp + tmppos, 0, TIGER_BLOCK_SIZE - tmppos);
#ifdef TIGER_BIG_ENDIAN
		for(j=0; j<TIGER_BLOCK_SIZE; j++)
			buf[j^7]=((uint8_t*)th->tmp)[j];
		tiger_compress_macro(((uint64_t*)buf), th->res);
		...
		tiger_compress_macro(((uint64_t*)th->tmp), th->res);
#endif
		memset(th->tmp, 0, TIGER_BLOCK_SIZE);
	} else {
		memset(th->tmp + tmppos, 0, TIGER_BLOCK_SIZE - tmppos - sizeof(uint64_t));
#ifdef TIGER_BIG_ENDIAN
		for(j=0; j<TIGER_BLOCK_SIZE; j++)
			buf[j^7]=((uint8_t*)th->tmp)[j];
		memcpy(th->tmp, buf, TIGER_BLOCK_SIZE);
#endif
	}

	((uint64_t*)(&(th->tmp[56])))[0] = th->pos<<3;
	tiger_compress_macro(((uint64_t*)(th->tmp)), th->res);
#ifdef TIGER_BIG_ENDIAN
	for(j = 0; j < TIGER_BYTES; j++)
		buf[j^7]=((uint8_t*)th->res)[j];
	memcpy(th->res, buf, TIGER_BYTES);
#endif
	return tigerhash_result(th);
}
